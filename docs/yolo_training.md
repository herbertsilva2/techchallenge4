...
- `razor_blade`: lamina/gilete potencialmente relevante para triagem humana.
- `box_cutter`: estilete potencialmente relevante para triagem humana.

As classes `knife` e `scissors` ja existem no modelo COCO usado pelo `yolov8n.pt`, por isso entram nas regras de risco sem exigir anotacao customizada nesta etapa.
